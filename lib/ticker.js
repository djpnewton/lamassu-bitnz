...
BitNZTicker.prototype.ticker = function ticker(currencies, callback) {
  if (currencies.length != 1) return callback(new Error('Unsupported currencies: ' + currencies.length));

  var currency = currencies[0];
  if (currency === 'NZD'){
    return fetchNZD(this.client, callback);
  } else {
    return callback(new Error(currency + ' is an unsupported currency'));
  }
